...
//				System.out.println(goalState.getTile(i, j));
				if (currentState.getTile(i, j) != goalState.getTile(i, j) && currentState.getTile(i, j) != 0)
					output++;
			}
		}
		return output;
	...
	public int computeH2(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				int temp = currentState.getTile(i, j);
				if (temp != 0) {
					int curTil=currentState.getTile(i, j);
					int goalTil=goalState.getTile(i, j);
					if (curTil!= goalTil) {
						int[] curPos=currentState.getLocation(curTil);
						int[] goalPos=goalState.getLocation(goalTil);
						result+=PuzzleUtils.manhattanDistance(curPos, goalPos);
					}
				}
			}
		}
		currentState.setH(result);
		return result;
	}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		else if (operator == 'r') {// Case-3: Move tile LEFT
			/* Enter your code here */
			int row = whiteTile[0];
		...
		else if (operator == 'l') {// Case-4: Move tile RIGHT
			/* Enter your code here */
			int row = whiteTile[0];
			int col = whiteTile[1] - 1;
